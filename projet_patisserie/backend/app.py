...
# ── Variables d'environnement (secrets Azure) ─────────────────────────────────
DATABASE_URL = os.environ.get("DATABASE_URL")
ADMIN_USER   = os.environ.get("ADMIN_USER", "admin")
ADMIN_PASS   = os.environ.get("ADMIN_PASS", "patisserie123")
...
def init_db():
    ...
    if count == 0:
        defaults = [
            ...
            ("Fondant Chocolat",       18,  "Gâteau au chocolat avec un coeur fondant et intense.",                         "https://i.pinimg.com/736x/fd/e4/75/fde4754d968d33518820dc8e61eb8920.jpg", "desserts"),
...
def update_product(pid):
    ...
    c.execute(
        ...
        (d.get("name", row["name"]).strip(), float(d.get("price", row["price"])),
         d.get("description", row["description"]).strip(), d.get("image", row["image"]),
         d.get("category", row["category"]), pid)
    )
    updated = c.fetchone(); conn.commit(); conn.close()
...
def delete_product(pid):
    ...
    if affected == 0: return jsonify({"error": "Introuvable"}), 404
...
def get_orders():
    ...
    orders = []
    for r in rows:
        o = dict(r); o["items"] = _json.loads(o["items"]); orders.append(o)
...
def create_order():
    ...
    address = d.get("address", "").strip(); items = d.get("items", [])
    errors = []
    ...
    conn = get_db(); c = conn.cursor()
    c.execute(
        "INSERT INTO orders (name, phone, address, items, total, status, date) VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING *",
        (name, phone, address, _json.dumps(items, ensure_ascii=False), total, "en attente", date)
    )
    row = c.fetchone(); conn.commit(); conn.close()
    o = dict(row); o["items"] = _json.loads(o["items"])
...
def update_order(oid):
    ...
    row = c.fetchone()
    if not row: conn.close(); return jsonify({"error": "Introuvable"}), 404
    c.execute("UPDATE orders SET status=%s WHERE id=%s", (d.get("status", row["status"]), oid))
